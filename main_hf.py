"""
CONNECT Project - Main Script - HuggingFace Version

This is the main entry point for the CONNECT project using HuggingFace models.
Handles both Phase 1 (training) and Phase 2 (testing/transformation).

Author: JM Patil
Date: November 11, 2025
Adapted for HuggingFace: January 31, 2026
"""
...
from llm_survey_hf import conduct_surveys, conduct_survey_single_story, sanitize_model_name, CostTracker
...
from story_transformer_hf import transform_story_iteratively
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='CONNECT Project - Narrative Transformation Pipeline (HuggingFace)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Phase 1: Train on individualistic stories (forward problem)
  python main_hf.py --phase 1 --problem forward --model mistralai/Mistral-7B-v0.3 --data-dir data --output-dir output

  # Phase 1: Train on collectivistic stories (inverse problem)
  python main_hf.py --phase 1 --problem inverse --model mistralai/Mistral-7B-v0.3 --data-dir data --output-dir output

  # Phase 2: Transform collectivistic story to individualistic
  python main_hf.py --phase 2 --problem forward --model mistralai/Mistral-7B-v0.3 --story data/test_story.txt

  # Phase 2: Transform individualistic story to collectivistic
  python main_hf.py --phase 2 --problem inverse --model mistralai/Mistral-7B-v0.3 --story data/test_story.txt
        """
    )
    ...
    parser.add_argument(
        ...
        help='HuggingFace model ID (default: mistralai/Mistral-7B-v0.3)'
    )
    ...
    # Log configuration
    logger.info("=" * 60)
    logger.info(f"CONNECT Project - Phase {args.phase} (HuggingFace)")
...
def run_phase1(args):
    ...
    This phase:
    1. Loads training stories
    2. Surveys them with LLM
    3. Learns PyReason rules from surveys
    ...
    # Step 1: Load training stories
    logger.info("\n[Step 1/3] Loading training stories...")
    ...
    logger.info("\n[Step 2/3] Conducting LLM surveys...")
    survey_results, failed_stories = conduct_surveys(
        stories=stories,
        questions_file=Path(args.data_dir) / "individualistic_questions.json" if args.problem == "forward" else Path(args.data_dir) / "collectivistic_questions.json",
        problem_type=args.problem,
        model=args.model,
        temperature=args.temperature,
        output_dir=args.output_dir
    )
    logger.info(f"✓ Survey complete: {len(survey_results)} succeeded, {len(failed_stories)} failed")

    # Step 3: Learn rules
    logger.info("\n[Step 3/3] Learning PyReason rules...")
    sanitized_model = sanitize_model_name(args.model)
    survey_dir = Path(args.output_dir) / sanitized_model / args.problem / "survey_results"
    rules_file = learn_rules(
        survey_results_dir=str(survey_dir),
        problem_type=args.problem,
        model=args.model,
        output_dir=args.output_dir
    )
    logger.info(f"✓ Rules learned and saved to: {rules_file}")

    # Summary
    logger.info("\n" + "=" * 60)
    logger.info("PHASE 1 COMPLETE")
    logger.info("=" * 60)
    logger.info(f"Stories surveyed: {len(survey_results)}")
    logger.info(f"Rules learned: {rules_file}")
    logger.info("=" * 60)


def run_phase2(args):
    """
    Execute Phase 2: Testing/Transformation - Transform test stories.
