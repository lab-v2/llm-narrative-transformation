"""
Story Transformer Module - HuggingFace Version

Transforms stories by rewriting segments identified by abduction analysis.
Uses HuggingFace models to iteratively modify segments to shift narrative style.
"""
...
from llm_survey_hf import CostTracker
...
def transform_story_iteratively(
    ...
    for i, presc in enumerate(selected_prescriptions, 1):
        ...
        try:
            # Call HF model
            rewritten_story, input_tokens, output_tokens = call_hf_with_retry(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 5:
        print("Usage: python story_transformer_hf.py <story.txt> <prescriptions.json> <top_k> <problem_type> <model>")
        print("Example: python story_transformer_hf.py story.txt ranked_prescriptions.json 3 forward mistralai/Mistral-7B-v0.3")
        sys.exit(1)
