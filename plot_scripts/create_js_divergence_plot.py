"""
JS Divergence Plotting Script

Creates publication-quality plots showing JS divergence between
original and transformed stories.

Lower JS divergence indicates better coherence with original narrative
...
HATCH_ABDUCTIVE = ['...', 'xxx', '\\\\\\', '|||', '+++']  # Different patterns
...
def load_js_divergence(
    ...
    Returns:
        DataFrame with JS divergences
    """
    sanitized_model = model.replace("/", "-").replace(":", "-")

    csv_file = Path(divergence_dir) / sanitized_model / problem_type / "js_divergence.csv"

    if not csv_file.exists():
        raise FileNotFoundError(f"JS divergence CSV not found: {csv_file}")
...
def create_js_divergence_plot(
    ...
    """
    Create JS divergence comparison plot.

    Args:
        df: DataFrame with JS divergences
    ...
    # Identify all JS columns
    all_cols = df.columns.tolist()
    baseline_col = 'baseline_js'
    abductive_cols = [col for col in all_cols if col.startswith('abductive_iter_') and col.endswith('_js')]
    abductive_cols = sorted(abductive_cols, key=lambda x: int(x.split('_')[2]))

    n_methods = 1 + len(abductive_cols)
    ...
    # Color gradient for abductive iterations
    def get_abductive_color(idx, total):
        if total == 1:
            return COLOR_ABDUCTIVE_BASE
        factor = idx / (total - 1)
        base_rgb = np.array([231, 76, 60]) / 255  # Light red
        dark_rgb = np.array([192, 57, 43]) / 255  # Dark red
...
def create_js_plots(
    ...
    """
    logger.info("=" * 60)
    logger.info("CREATING JS DIVERGENCE PLOT")
    logger.info("=" * 60)
    logger.info(f"Model: {model}")
    logger.info(f"Problem: {problem_type}")

    # Load data
    df = load_js_divergence(model, problem_type, divergence_dir)
    ...
    # Create plot
    output_file = plot_dir / "js_divergence.png"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Create JS divergence plots',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Create plot for gpt-4o forward
  python plot_scripts/create_js_divergence_plot.py --model gpt-4o --problem forward

  # Create for Claude inverse
  python plot_scripts/create_js_divergence_plot.py --model claude-sonnet-4-5 --problem inverse
        """
    )
