"""
Phase 1 Cost Comparison Plotting Script

Creates box plots comparing LLM costs across models for Phase 1.
Generates 4 plots: input tokens, output tokens, total tokens, and cost.
...
def load_phase1_costs(
        models: list,
        problem_type: str,
        costs_dir: str = "output_analysis/costs/phase1"
) -> pd.DataFrame:
    """
    Load Phase 1 costs for all models.
...
def create_cost_boxplot(
    ...
    Args:
        ...
        metric: Column name ('input_tokens', 'output_tokens', 'total_tokens', 'cost_usd')
        ylabel: Y-axis label
    ...
    ax.set_title(
        f'Phase 1 {ylabel} Distribution Across Models\n{problem_desc}',
...
def create_phase1_cost_plots(
    ...
    """
    Create Phase 1 cost comparison plots.
    ...
    """
    logger.info("=" * 60)
    logger.info("CREATING PHASE 1 COST PLOTS")
    logger.info("=" * 60)
    logger.info(f"Models: {models}")
    logger.info(f"Problem: {problem_type}")

    # Load data
    df = load_phase1_costs(models, problem_type, costs_dir)
    ...
    for metric_col, metric_label in metrics:
        ...
        output_file = plot_dir / f"phase1_{metric_col}_boxplot.png"
    ...
    logger.info("\n" + "=" * 60)
    logger.info("PHASE 1 COST PLOTS COMPLETE")
    logger.info("=" * 60)
    logger.info(f"Output: {plot_dir}")
    logger.info("Files created:")
    for metric_col, _ in metrics:
        logger.info(f"  - phase1_{metric_col}_boxplot.png")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Create Phase 1 cost comparison plots',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Compare all 4 models for forward problem
  python plot_scripts/plot_phase1_costs.py \\
    ...
    parser.add_argument('--costs-dir', default='output_analysis/costs/phase1')
