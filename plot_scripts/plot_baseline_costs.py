"""
Baseline Cost Comparison Plotting Script

Creates box plots comparing LLM costs across models for baseline approach.
Generates plots for each cost metric column.
...
def load_baseline_costs(
        models: list,
        problem_type: str,
        costs_dir: str = "output_analysis/costs/baseline"
) -> pd.DataFrame:
    """
    Load baseline costs for all models.
...
def create_cost_boxplot(
    ...
    ax.set_title(
        f'Baseline {ylabel} Distribution Across Models\n{problem_desc}',
...
def create_baseline_cost_plots(
        models: list,
        problem_type: str,
        costs_dir: str = "output_analysis/costs/baseline",
    ...
    """
    Create baseline cost comparison plots.
    ...
    """
    logger.info("=" * 60)
    logger.info("CREATING BASELINE COST PLOTS")
    logger.info("=" * 60)
    logger.info(f"Models: {models}")
    logger.info(f"Problem: {problem_type}")

    # Load data
    df = load_baseline_costs(models, problem_type, costs_dir)
    ...
    metrics = [
        ('transformation_input_tokens', 'Transformation Input Tokens'),
        ('transformation_output_tokens', 'Transformation Output Tokens'),
        ('transformation_cost_usd', 'Transformation Cost (USD)'),
        ('post_survey_input_tokens', 'Post-Survey Input Tokens'),
        ('post_survey_output_tokens', 'Post-Survey Output Tokens'),
        ('post_survey_cost_usd', 'Post-Survey Cost (USD)'),
    ...
    for metric_col, metric_label in metrics:
        ...
        output_file = plot_dir / f"baseline_{metric_col}_boxplot.png"
    ...
    logger.info("\n" + "=" * 60)
    logger.info("BASELINE COST PLOTS COMPLETE")
    logger.info("=" * 60)
    logger.info(f"Output: {plot_dir}")
    logger.info("Files created:")
    for metric_col, _ in metrics:
        logger.info(f"  - baseline_{metric_col}_boxplot.png")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Create baseline cost comparison plots',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Compare all 4 models for forward problem
  python plot_scripts/plot_baseline_costs.py \\
    ...
    parser.add_argument('--costs-dir', default='output_analysis/costs/baseline')
